...
minGL &rectangle::_Edit(minGL &Window) const throw (myexception)
{
    ...
    //l'interieur
    for (unsigned x (botLeft.abs + 1); x < botRight.abs; ++x)
        for (unsigned y(botLeft.ord +1); y < topLeft.ord; ++y)
...
rectangle::rectangle(const pos &pos1, const pos &pos2, const RGBcolor &borderCol, const RGBcolor &inCol)
{
    ...
    remarkablePt ["topLeft"]  = pos (max (pos1.abs, pos2.abs), min (pos1.ord, pos2.ord));
...
rectangle::rectangle(const pos &pos_, const int &width, const int &height, const RGBcolor &borderCol, const RGBcolor &inCol)
{
